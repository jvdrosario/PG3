/*------------------------------------------
    ...
    Student Author: Joshua Del Rosario
    Lab Section: Tuesday 11PM
-------------------------------------------*/
...
//The auto-grader will change the random seed
#ifndef SEED_VALUE
#define SEED_VALUE 0
#endif
        ...
        for (int y=i-1; y<=i+1; y++) {
            for (int x=j-1; x<=j+1; x++) {
                if (!(x < 0 || y < 0) && !(x > 4 || y > 7)) {
                    if (isMine(y, x)) {
                        mines_nearby++;
                        cout << isMine(y, x) << " " << x << " " << y << " " << mines_nearby << endl;
                    }
                }
            }
        } setCell(i, j, mines_nearby);
    }
}
